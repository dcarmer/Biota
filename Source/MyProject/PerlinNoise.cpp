...
PerlinNoise::PerlinNoise(int seed)
{
	...
	{
		FMath::SinCos(&(Gradient[i].X), &(Gradient[i].Y), RStream.FRandRange(0, 2 * PI));//NOTE[IMPROVE]: Inclusive, so 0=360 counted twice
	}
}
...
float PerlinNoise::GetNoiseAt(float x, float y)
{
...
float PerlinNoise::GetFractalNoiseAt(float x, float y, float persistence, int octives)
{
